...
public static class ContractQueryMapping
{
    public static void MapFromCustomer(List<Customer> models, object mappedObject, IMapper mapper)
    {
        if (mappedObject is DatabaseEntity.Contract)
        {
            ...
            var index = models.Where(c => c.CustomerBankingRelationship != null).ToList().FindIndex(c =>
                c.CustomerBankingRelationship.Any(cbr => cbr.ContractKey
            ...
            if (index >= 0)
            {
                ...
                if (indexCbs >= 0)
                {
                    models[index].CustomerBankingRelationship[indexCbs].Contract =
                        models[index].CustomerBankingRelationship[indexCbs].Contract ?? [];
