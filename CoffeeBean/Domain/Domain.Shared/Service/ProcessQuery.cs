    ...
    where M : class, new() where D : class  where S : class
{
        ...
        ExecuteAsync(SqlStructure parameters, CancellationToken cancellationToken)
    {
        ...
        var queryParameters = QueryHelper.GetParameters(_treeMap.NodeTree, parameters);
        ...
        try
        {
            var result =
                await connection.QueryAsync<(int? startCursor, int? endCursor, int? totalCount, int? totalPageRecords)>(
                    ...
                    }, queryParameters, splitOn: splitOn, commandType: CommandType.Text);
            ...
            return (_models,
                ...
                result.Select(s => s.totalCount).FirstOrDefault(), result.Select(s => s.totalPageRecords).FirstOrDefault());
        }
        catch (Exception ex)
        {
